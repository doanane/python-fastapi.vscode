...
class main(BaseModel):  # create a class that inherits from BaseModel, you can change name of class to anything like Book, Item etc
    id: UUID
...
@app.put("/{book_id}")
def update_book(book_id: UUID, book: main): #  we want to update a book by its id, so we need to pass the id as a path parameter    
    for index, b in enumerate(BOOKS): #  we need to loop through the list of books to find the book with the given id. #  use enumerate to get the index of the book in the list #  then we can update the book at that index #  then return the updated book #  if the book is not found, return an error message
        if b.id == book_id:
            BOOKS[index] = book
            return BOOKS[index]
